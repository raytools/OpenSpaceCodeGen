    ...
    public static class OperatorTranslation
    {
        ...
        public static NodeTranslator TranslateOperator(NodeOperator node, CodeGenerator generator)
        {
            ...
            switch (op) {
                ...
                case EnumOperator.Operator_Ultra: return NodeTranslator.Sequence(0, ".", 1); // should be ".." according to source, but can't find any scripts with that?
        ...
        public static bool IsAssignmentOperator(EnumOperator op)
        {
            return op == EnumOperator.Operator_Affect || op == EnumOperator.Operator_AffectArray ||
                   op == EnumOperator.Operator_DivAffect || op == EnumOperator.Operator_MinusAffect ||
                   op == EnumOperator.Operator_MulAffect || op == EnumOperator.Operator_PlusAffect ||
                   op == EnumOperator.Dot_X_Assign || op == EnumOperator.Dot_Y_Assign || op == EnumOperator.Dot_Z_Assign;
        }
    }
}