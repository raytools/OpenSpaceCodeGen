...
namespace OpenSpaceCodeGen.Translation.CBase.CSharp {
    ...
    public class LanguageTranslationCSharp : LanguageTranslationCBase
    {
        ...
        {
            if (node is NodeComportRef || node is NodeSubRoutine || node is NodeString || node is NodeDsgVarRef) {
                return NodeTranslator.Sequence(gen.ReferenceResolver.ResolveFunc.Invoke(node));
            }

            if (node is NodeRealRef || node is NodeConstantRef) {
               return NodeTranslator.Sequence("Defines."+gen.ReferenceResolver.ResolveFunc.Invoke(node));
            }
